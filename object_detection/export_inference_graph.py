...
def main(_):
    ...
    input_type = 'image_tensor'
    ckpt = 'models/model/train/model.ckpt-' + get_ckpt()
    if os.path.exists('output_graph') == True: shutil.rmtree('output_graph')
    output_dir = 'output_graph'

    pipeline_config = pipeline_pb2.TrainEvalPipelineConfig()
    with tf.gfile.GFile('models/model/ssd_mobilenet_v1_coco.config', 'r') as f:
        text_format.Merge(f.read(), pipeline_config)
    exporter.export_inference_graph('image_tensor', pipeline_config, ckpt, output_dir)
